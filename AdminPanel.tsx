...
import { Category, JobEntry, SiteSettings } from '../types';
import { addJob, updateJob, deleteJob, saveSiteSettings, subscribeToJobs, subscribeToSettings } from '../db';
import { syncPostFromLink, searchLatestJobsFromWeb } from '../geminiService';
...
const AdminPanel: React.FC<AdminPanelProps> = ({ onUpdate }) => {
  ...
  const [formData, setFormData] = useState<Partial<JobEntry>>({
    ...
    link: '', isUrgent: false, status: 'published'
  });
  ...
  const handleSettingsSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert('Settings & AdSense Configuration Saved!');
    } catch (e) {
  ...
  if (!isAuthenticated) {
    return (
      <div className="flex justify-center py-20 px-4">
        <div className="bg-white p-8 rounded-xl shadow-2xl border-t-8 border-[#ff0000] w-full max-sm">
    ...
    <div className="bg-white rounded-xl shadow-2xl border-2 border-[#000080] overflow-hidden">
      <div className="bg-[#000080] text-white p-4 flex flex-wrap gap-2 justify-between items-center">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={handleDiscoverJobs} className="bg-yellow-500 text-black px-4 py-2 rounded font-black text-[10px] uppercase">🔍 AI Discover</button>
      ...
      <div className="p-4 md:p-8">
          ...
          <form onSubmit={handleSettingsSave} className="space-y-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div className="bg-gray-50 p-6 rounded-xl border-2 border-gray-200 space-y-4">
                <h4 className="font-black text-[#ff0000] uppercase text-sm mb-2">General Configuration</h4>
                <div>
                  <label className="block text-[10px] font-black uppercase mb-1">Website Name</label>
                  <input type="text" className="w-full p-3 border-2 rounded font-bold" value={settingsForm.siteName} onChange={(e) => setSettingsForm({...settingsForm, siteName: e.target.value})} />
                </div>
                <div>
                  <label className="block text-[10px] font-black uppercase mb-1">Telegram Link</label>
                  <input type="text" className="w-full p-3 border-2 rounded font-bold" value={settingsForm.telegramLink} onChange={(e) => setSettingsForm({...settingsForm, telegramLink: e.target.value})} />
                </div>
                <div>
                  <label className="block text-[10px] font-black uppercase mb-1">WhatsApp Link</label>
                  <input type="text" className="w-full p-3 border-2 rounded font-bold" value={settingsForm.whatsappLink} onChange={(e) => setSettingsForm({...settingsForm, whatsappLink: e.target.value})} />
                </div>
              </div>

              {/* AdSense Configuration */}
              <div className="bg-blue-50 p-6 rounded-xl border-2 border-blue-200 space-y-4">
                <h4 className="font-black text-blue-800 uppercase text-sm mb-2">Google AdSense Configuration</h4>
                <div>
                  <label className="block text-[10px] font-black uppercase mb-1">Publisher ID (ca-pub-xxx)</label>
                  <input type="text" placeholder="ca-pub-..." className="w-full p-3 border-2 rounded font-bold border-blue-100" value={settingsForm.publisherId} onChange={(e) => setSettingsForm({...settingsForm, publisherId: e.target.value})} />
                </div>
                <div>
                  <label className="block text-[10px] font-black uppercase mb-1">Top Ad Slot ID</label>
                  <input type="text" className="w-full p-3 border-2 rounded font-bold border-blue-100" value={settingsForm.adSlotTop} onChange={(e) => setSettingsForm({...settingsForm, adSlotTop: e.target.value})} />
                </div>
                <div>
                  <label className="block text-[10px] font-black uppercase mb-1">Bottom Ad Slot ID</label>
                  <input type="text" className="w-full p-3 border-2 rounded font-bold border-blue-100" value={settingsForm.adSlotBottom} onChange={(e) => setSettingsForm({...settingsForm, adSlotBottom: e.target.value})} />
                </div>
                <div>
                  <label className="block text-[10px] font-black uppercase mb-1">Sidebar Ad Slot ID</label>
                  <input type="text" className="w-full p-3 border-2 rounded font-bold border-blue-100" value={settingsForm.adSlotSide} onChange={(e) => setSettingsForm({...settingsForm, adSlotSide: e.target.value})} />
                </div>
              </div>
            </div>
            <button type="submit" className="w-full py-4 bg-green-600 text-white font-black rounded-lg uppercase shadow-xl hover:bg-green-700 transition">💾 SAVE ALL SETTINGS</button>
          </form>
        )}
        ...
        {activeTab === 'discover' && (
          <div className="space-y-4">
            ...
            {isAiLoading ? (
              <div className="p-10 text-center font-black animate-pulse">AI IS SEARCHING FOR NEW NOTIFICATIONS...</div>
            ) : (
              <div className="grid grid-cols-1 gap-3">
                {discoveredJobs.map((dj, i) => (
                  <div key={i} className="p-4 border-2 border-gray-100 rounded-lg flex justify-between items-center bg-gray-50">
                    <div>
                    ...
                    <button onClick={() => handleSyncThisJob(dj.url)} className="bg-green-600 text-white px-4 py-2 rounded font-black text-[10px] uppercase">Sync & Edit</button>
                  </div>
                ))}
              </div>
            )}
        ...
        {activeTab === 'create' && (
          <form onSubmit={handleSubmit} className="space-y-6">
             <div className="bg-blue-50 p-4 border-2 border-dashed border-blue-200 rounded-lg flex gap-2">
               <input type="text" placeholder="Paste Official Job URL to Auto-Fill with AI" className="flex-1 p-3 border-2 rounded font-bold text-sm" value={formData.link} onChange={(e) => setFormData({...formData, link: e.target.value})} />
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <input type="text" placeholder="Last Date" className="p-3 border-2 rounded font-bold" value={formData.lastDate} onChange={(e) => setFormData({...formData, lastDate: e.target.value})} />
             </div>
             <textarea placeholder="AI will automatically write description here..." className="w-full p-3 border-2 rounded font-bold h-32" value={formData.shortInfo} onChange={(e) => setFormData({...formData, shortInfo: e.target.value})} />
             <button type="submit" className="w-full py-5 bg-red-600 text-white font-black text-xl rounded shadow-xl uppercase">Publish To Website</button>
          </form>
        )}
                ...
                {jobs.map(j => (
                  <tr key={j.id} className="hover:bg-gray-50">
                    ...
                    <td className="p-4 text-right">
                      <button onClick={() => { setFormData(j); setEditingId(j.id); setActiveTab('create'); }} className="text-blue-600 font-black text-[10px] mr-4 uppercase">Edit</button>
                      <button onClick={() => deleteJob(j.id)} className="text-red-600 font-black text-[10px] uppercase">Delete</button>
                    </td>
                  </tr>
                ))}
              </tbody>
